...
![image](https://github.com/simon0018/DB_SBER_HOMEWORK_4/assets/89707757/6e3b8136-c2f4-40d8-af00-34bcc393f404)
Этот запрос открывает базу данных "example" и ищет все элементы с именем "book", затем возвращает их.

Получить все книги, написанные автором "George Orwell"
![image](https://github.com/simon0018/DB_SBER_HOMEWORK_4/assets/89707757/a1f5eee2-4f88-4b87-b772-7766f4d484ba)

Получить список всех книг в алфавитном порядке
![image](https://github.com/simon0018/DB_SBER_HOMEWORK_4/assets/89707757/5e462241-2c25-4388-8278-8122829c9bfe)
...
![image](https://github.com/simon0018/DB_SBER_HOMEWORK_4/assets/89707757/78e7c251-0064-4e3c-a275-d1ebf7b7aca6)
Где:
...
![image](https://github.com/simon0018/DB_SBER_HOMEWORK_4/assets/89707757/44494199-8901-43a2-b914-8a5b9df558c0)

После создания индекса можно использовать его в запросах, например:
![image](https://github.com/simon0018/DB_SBER_HOMEWORK_4/assets/89707757/ac07ca88-fef7-4e85-a9dd-284e430b5dee)
...
В процессе выполнения запросов BaseX использует индексы и другие оптимизации для ускорения выполнения запросов. BaseX также поддерживает распределенные вычисления и параллельное выполнение запросов для повышения производительности.
...
• Блокировки не могут быть синхронизированы между экземплярами BaseX, которые запущены в разных JVM. Если требуется выполнять параллельные операции записи, мы обычно рекомендуем работать с клиент-серверной или HTTP-архитектурой.
• Неожиданное завершение работы сервера во время транзакции, вызванное сбоем оборудования или отключением питания, может привести к несогласованному состоянию базы данных, если транзакция была активна во время выключения. Рекомендуется использовать команду CREATE BACKUP для регулярного резервного копирования вашей базы данных. Если произошел наихудший случай, вы можете попробовать команду INSPECT, чтобы проверить, есть ли явные несоответствия в вашей базе данных, и использовать команду RESTORE для восстановления последней резервной копии базы данных.
...
Да, термины Data Mining, Data Warehousing и OLAP могут быть применены в контексте работы с BaseX.

Data Mining (англ. "добыча данных") - это процесс автоматического поиска скрытых закономерностей в больших объемах данных. BaseX может использоваться для хранения и обработки больших объемов данных, что делает его подходящим инструментом для выполнения задач Data Mining.

Data Warehousing (англ. "хранилище данных") - это процесс сбора, хранения и управления данными в целях их анализа и использования в бизнес-процессах. BaseX может использоваться как основа для создания хранилища данных, так как он поддерживает эффективное хранение и быстрый доступ к большим объемам данных.

OLAP (англ. "анализ данных в режиме реального времени") - это процесс анализа данных в режиме реального времени с целью получения бизнес-информации. BaseX поддерживает OLAP-анализ данных с помощью функций XQuery, которые позволяют осуществлять многомерный анализ данных и создавать отчеты на основе запросов к базе данных.
...
3. Поддержка различных режимов безопасности: BaseX может работать в различных режимах безопасности, таких как режим "безопасный для записи" и режим "безопасный для чтения". Режим "безопасный для записи" ограничивает доступ к базе данных только для чтения, а режим "безопасный для чтения" позволяет только чтение, но не запись в базу данных.

4. Защита от инъекций: BaseX предотвращает атаки инъекций с помощью встроенных механизмов безопасности, таких как экранирование символов и использование параметризованных запросов.

5. Аудит действий: BaseX предоставляет журнал аудита, который содержит записи обо всех действиях пользователей в базе данных, таких как вход, выход, запросы и изменения данных.

В целом, BaseX обеспечивает надежную защиту баз данных и предоставляет различные методы для обеспечения безопасности и конфиденциальности данных.
